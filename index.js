...
getPokemonData.then(() => {
  function attack() {
    if (!isWaiting) {
      ...
      document.getElementById("playerTeam").innerHTML =
        playerTeam[0].getCharacterHtml();
      document.getElementById("enemyTeam").innerHTML =
        enemyTeam[0].getCharacterHtml();
  ...
  function endGame() {
    isWaiting = true;
    const endMessage =
      playerTeam[0].health === 0 && enemyTeam[0].health === 0
        ? "Game Over!"
        : playerTeam[0].health > 0
        ? `${
            playerTeam[0].name.charAt(0).toUpperCase() +
            playerTeam[0].name.slice(1)
          } wins!`
        : `${
            enemyTeam[0].name.charAt(0).toUpperCase() +
            enemyTeam[0].name.slice(1)
          } wins!`;
